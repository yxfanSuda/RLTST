...
if __name__ == '__main__':
    ...
    parser.add_argument('--TSTRL_model_path', type=str, default='model.pt')
    ...
    def train_collate_fn_hu_rl(examples):
        ...
        def pool(d):
            ...
            i, j, t = 0, 0, 0
            for edu_num in edu_nums:
                if t + edu_num > args.hu_batch_size_rl:
                    buckets.append((i, j))
                    i, t = j, 0
                t += edu_num
                j += 1
            ...
            for bucket in buckets:
                ...
                texts, input_mask, segment_ids, none_, sep_index, pairs, graphs, speakers, turns, edu_nums, ex_ids = zip(
                    *batch)
                texts = torch.stack(texts, dim=0)
                segment_ids = torch.stack(segment_ids, dim=0)
                input_mask = torch.stack(input_mask, dim=0)
                ...
                graphs = list(graphs)
                edu_nums = torch.tensor(edu_nums)
                yield texts, input_mask, segment_ids, list(none_), list(sep_index), list(
                    pairs), graphs, speakers, turns, edu_nums, list(ex_ids)

        return pool(examples)


    def train_collate_fn_ou_len5_rl(examples):
        ...
        def pool(d):
            ...
            i, j, t = 0, 0, 0
            for edu_num in edu_nums:
                if t + edu_num > args.ou5_batch_size_rl:
                    buckets.append((i, j))
                    i, t = j, 0
                t += edu_num
                j += 1
    ...
    def train_collate_fn_ou_len10_rl(examples):
        ...
        def pool(d):
            ...
            i, j, t = 0, 0, 0
            for edu_num in edu_nums:
                if t + edu_num > args.ou10_batch_size_rl:
                    buckets.append((i, j))
                    i, t = j, 0
                t += edu_num
                j += 1
    ...
    def train_collate_fn_ou_len15_rl(examples):
        ...
        def pool(d):
            ...
            i, j, t = 0, 0, 0
            for edu_num in edu_nums:
                if t + edu_num > args.ou15_batch_size_rl:
                    buckets.append((i, j))
                    i, t = j, 0
                t += edu_num
                j += 1
    ...
    def SingleTaskLearning(mtl_model,  train_mol_dataloader):
        step = 0
    ...
    def RL_Training(policy_model, task_type_list, traindata_loader_list, eval_data, ex_id_des_file=None, optim_rel=False):
        step_rl = 0
        save_step = 1
        init_reward = -1000
        with open(ex_id_des_file,'a+',encoding='utf8')as fw:
            for epoch in range(args.RL_epoches):
                print('{} epoch training..'.format(epoch + 1))
                model.train()
                for index, train_dataloader in enumerate(traindata_loader_list):
                    for train_batch in tqdm(train_dataloader):
                        for mini_batch in train_batch:
                            texts_ids = mini_batch[0].cuda()
                            text_mask = mini_batch[1].cuda()
                            segmend_ids = mini_batch[2].cuda()
                            action_probs, batch_rep, exp_reward = policy_model(texts_ids, text_mask, segmend_ids)
                            m = Categorical(action_probs)
                            action = m.sample()
                            batch_mask = action.cpu() == torch.ones(action.size())
                            if torch.sum(batch_mask).item() > 0:
                                select_index_list = get_index_list(batch_mask)
                                select_mini_batch = []
                                for item in mini_batch:
                                    if isinstance(item, list):
                                        select_mini_batch.append(select_from_list(item, select_index_list))
                                    else:
                                        select_mini_batch.append(item[select_index_list])
                                select_mini_batch[6] = ints_to_tensor(select_mini_batch[6])
                                select_mini_batch[7] = ints_to_tensor(select_mini_batch[7])
                                select_mini_batch[8] = ints_to_tensor(select_mini_batch[8])
                                if optim_rel:
                                    policy_model.train_minibatch_rl(task_type_list[index], select_mini_batch)
                                else:
                                    policy_model.train_minibatch_optim_link_rl(task_type_list[index], select_mini_batch)
                                    
                                _, reward = policy_model.compute_f1_and_loss_reward_rl(task_type_list[index], eval_data)
                                
                                if reward > init_reward:
                                    init_reward = reward
                                    for ex_id in select_mini_batch[-1]:
                                        fw.write(ex_id + '\n')
    ...
    def select_from_list(items_list, index_list):
        new_item_list = []
        for index in index_list:
            new_item_list.append(items_list[index])
        return new_item_list

    def get_index_list(mask):
        index_list = []
    ...
    if args.do_train:
        ...
        for epoch in range(args.ST_epoches):
            print('{} epoch ST pre-training..'.format(epoch + 1))
            model.train()
            SingleTaskLearning(model,  train_dataloader_mol)
                                                  
            mol_linkandrel_loss, _ = model.compute_f1_and_loss_reward(tasktype='parsing', eval_dataloader = eval_dataloader_mol)
        ...
        train_dataloader_hu = DataLoader(dataset=train_dataset_hu, batch_size=args.hu_pool_size, shuffle=True,
                                         collate_fn=train_collate_fn_hu_rl)
    
        data_loader_list.append(train_dataloader_hu)
        train_dataloader_ou5 = DataLoader(dataset=train_dataset_ou5, batch_size=args.ou5_pool_size,
                                          shuffle=True,
                                          collate_fn=train_collate_fn_ou_len5_rl)
        data_loader_list.append(train_dataloader_ou5)

        train_dataloader_ou10 = DataLoader(dataset=train_dataset_ou10, batch_size=args.ou10_pool_size,
                                           shuffle=True,
                                           collate_fn=train_collate_fn_ou_len10_rl)
        data_loader_list.append(train_dataloader_ou10)

        train_dataloader_ou15 = DataLoader(dataset=train_dataset_ou15, batch_size=args.ou15_pool_size,
                                           shuffle=True,
                                           collate_fn=train_collate_fn_ou_len15_rl)
        data_loader_list.append(train_dataloader_ou15)
        selected_files = [args.hu_select_id_file, args.ou5_select_id_file, args.ou10_select_id_file, args.ou15_select_id_file]

        for data, sele_file, in zip(data_loader_list, selected_files):
            args.TST_Learning_Mode = False
            model = PolicyNetwork(args=args, pretrained_model=pretrained_model)
            model = model.to(args.device)
            state_dict = torch.load(args.ST_model_path+'.pt')
            ...
            for k, v in model.named_parameters():
                if k.startswith('critic.task_model.base_network.pretrained_model.embeddings'):
                    v.requires_grad = False
            for k, v in model.named_parameters():
                if k.startswith('critic.task_model.base_network.pretrained_model.pooler'):
                    v.requires_grad = False
            for layer in range(12):
                for k, v in model.named_parameters():
                    if k.startswith('critic.task_model.base_network.pretrained_model.encoder.layer.{}.'.format(layer)):
                        v.requires_grad = False
            ...
            optimizer = optim.Adadelta(params=param_groups, lr=args.learning_rate)
            task_type_list = ['parsing']
            traindata_loader_list = [data]
            RL_Training(model, task_type_list, traindata_loader_list, eval_dataloader_mol, sele_file, optim_rel=False)
        
        selected_files = [args.hu_select_id_file, args.ou5_select_id_file, args.ou10_select_id_file, args.ou15_select_id_file]
        src_files = [args.train_hu_file, args.train_ou5_file, args.train_ou10_file, args.train_ou15_file]
        des_files = [args.hu_selected_data_file, args.ou5_selected_data_file, args.ou10_selected_data_file, args.ou15_selected_data_file]
        for id_file, src_file, des_file in zip(selected_files, src_files, des_files):
            write_selected_data(id_file=id_file, src_file = src_file, des_file = des_file)